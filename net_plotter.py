...
# p(quantize(layer.weight, layer.num_bits_weight, min_value=float(layer.running_min), max_value=float(layer.running_max), noise=False,eta=0.))
def get_quantized_weights(net):
    net.eval()
    """ Extract parameters from net, and return a list of tensors"""
    param_list = []
    for l, (name, layer) in enumerate(net.named_modules()):
        if ('conv' in name or 'fc' in name) and 'input' not in name:
            # print(name)
            with torch.no_grad():
                qweight = quantize(layer.weight, layer.num_bits_weight, min_value=float(layer.running_min),
                                   max_value=float(layer.running_max), noise=False, eta=0.)

                param_list.append(qweight)
    # exit()
    return param_list

# def get_quantized_weights(quantized_net):
...
def set_weights(net, weights, directions=None, step=None):
    ...
    SET_QTHETA = FLAGS.loss_surf_eval_d_qtheta
    ...
    if directions is None:
        ...
        if SET_QTHETA:
            ...
            for l, (name, layer) in enumerate(net.named_modules()):
                ...
                if ('conv' in name or 'fc' in name) and 'input' not in name:
                    with torch.no_grad():
                        layer.d_theta= torch.Tensor(changes[i]).type(layer.weight.type())
                        i+=1
        else:
            for (p, w, d) in zip(net.parameters(), weights, changes):
                p.data = w + torch.Tensor(d).type(w.type())
...
def create_random_direction(net, dir_type='weights', ignore='biasbn', norm='filter'):
    ...
    # random direction
    if dir_type == 'weights':
        if FLAGS.loss_surf_eval_d_qtheta:
            weights = get_quantized_weights(net)
        else:
